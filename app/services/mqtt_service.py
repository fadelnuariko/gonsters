...
class MQTTService:
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            logger.info("Connected to MQTT broker successfully", extra={
                'extra_data': {
    ...
    def on_disconnect(self, client, userdata, rc):
        ...
        self.is_connected = False
        if rc != 0:
            logger.warning("Unexpected MQTT disconnection. Attempting to reconnect...", extra={
                'extra_data': {'return_code': rc}
            })
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            ...
            logger.info(f"MQTT message received from {msg.topic}", extra={
                'extra_data': {
                    'topic': msg.topic,
                    'factory': factory_id,
            ...
            if not self._validate_payload(payload):
                logger.warning("Invalid payload structure", extra={
            ...
            logger.info(f"Successfully ingested data from MQTT for machine {data_point['machine_id']}")

        except json.JSONDecodeError as e:
            logger.error(f"Failed to decode MQTT message: {e}", extra={
                'extra_data': {'payload': msg.payload}
            })
